...
export class WireConditionEquip extends BaseEquip {
  ...
  init() {
    ...
    if (['on', 'off'].includes(keyValue)) {
      ...
      // this.setTemperature(+this.getTemperature(new this.Converter(`${WireConditionEquip.defaultTemp}`, 10).toHex()))
    } else {
  ...
  setMode(mode: number): WireConditionEquip {
    ...
    // 自动、抽湿 无温度，否则默认温度
    // this.setTemperature([1, 3].includes(mode) ? 0 : WireConditionEquip.defaultTemp)
   ...
   * @param mode 模式
   */
  setPowerOn(temp: number = 26, speed: number = 0, mode: number = 1): WireConditionEquip {
